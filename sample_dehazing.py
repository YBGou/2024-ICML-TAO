...
class GANLoss(nn.Module):
    def __init__(self):
        ...
        self.optimizer = th.optim.Adam(self.disc_net.parameters(), 5e-4)
...
class GenerativeDegradation(nn.Module):
    ...
    def forward(self, input, target):
        ...
        mse = self.mse_loss(output, input.detach()) * 0.2
        pse = self.perceptual_loss(output, input.detach()) * 0.5
        adv = self.adversarial_loss(output, target.detach()) * 0.3
...
def main():
    ...
    def general_cond_fn(x, t, y, x_img=None):
        ...
        with th.enable_grad():
            ...
            warmup = 2 * (1 - (t[0].item() - 700) / 299)
            mse = mse_loss(x_in_lq, x_tg_lq.detach()) * warmup if t[0] >= 700 else 0

            # Second Stage (t ~ [700, 0])
            mse = mse_loss(x_in_lq[:args.batch_size],
                           x_tg_lq[:args.batch_size].detach()) * 1 if t[0] < 700 else mse
            pse = perceptual_loss(x_in_lq[:args.batch_size],
                                  x_tg_lq[:args.batch_size].detach()) * 1e-3 if t[0] < 700 else 0
            adv = 0 #adversarial_loss(x_in_hq[:args.batch_size] - x_tg_lq[:args.batch_size].detach(),
                                #    (x_in_hq - x_in_lq).detach()) * 1e-4 if t[0] < 700 else 0
            
            # Third Stage (t ~ [50, 0])
            tv = tv_loss(x_in_hq[:args.batch_size]) * 1e-4 if t[0] < 50 else 0
...
def create_args():
    result_dir = f"test_samples/HSTS_256x256/results"
    sample_dir = f"test_samples/HSTS_256x256/synthetic"
    ...
    parser.add_argument("--guidance_scale", type=int, default=25000)
