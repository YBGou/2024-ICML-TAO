...
class GANLoss(nn.Module):
    def __init__(self):
        ...
        self.optimizer = th.optim.Adam(self.disc_net.parameters(), 5e-5)
...
class GenerativeDegradation(nn.Module):
    ...
    def forward(self, input, target):
        ...
        pse = self.perceptual_loss(output, input.detach()) * 0.3
        adv = self.adversarial_loss(output, target.detach()) * 0.6
...
def main():
    ...
    def general_cond_fn(x, t, y, x_img=None):
        ...
        with th.enable_grad():
            ...
            # Third Stage (t ~ [50, 0])
            tv = tv_loss(x_in_hq[:args.batch_size]) * 4e-5 if t[0] < 50 else 0
...
def create_args():
    result_dir = f"test_samples/LOL_256x256/results"
    sample_dir = f"test_samples/LOL_256x256/lowlight"
    ...
    parser.add_argument("--guidance_scale", type=int, default=50000)
